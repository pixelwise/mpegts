    ...
    while (!in.empty()) {
        ...
        if (_ts_frames.find(ts_header.pid) != _ts_frames.end()) {
            ...
            if (ts_header.adaptation_field_control == MpegTsAdaptationFieldType::payload_only ||
                ...
                if (ts_header.payload_unit_start_indicator == 0x01) {
                    ...
                    if (pes_header.pes_packet_length != 0) {
                        if ((pes_header.pes_packet_length - 3 - pes_header.header_data_length) >= 188 || (pes_header.pes_packet_length - 3 - pes_header.header_data_length) < 0) {
                            _ts_frames[ts_header.pid]->data->append(in.data() + in.pos(), 188 - (in.pos() - pos));
                        } else {
                            _ts_frames[ts_header.pid]->data->append(in.data() + in.pos(), pes_header.pes_packet_length - 3 - pes_header.header_data_length);
                        }
                ...
                if(_ts_frames[ts_header.pid]->expected_pes_packet_length != 0 && _ts_frames[ts_header.pid]->data->size() + 188 - (in.pos() - pos) > _ts_frames[ts_header.pid]->expected_pes_packet_length) {
                    _ts_frames[ts_header.pid]->data->append(in.data() + in.pos(), _ts_frames[ts_header.pid]->expected_pes_packet_length - _ts_frames[ts_header.pid]->data->size());
                } else {
                    _ts_frames[ts_header.pid]->data->append(in.data() + in.pos(), 188 - (in.pos() - pos));
                }
            }
